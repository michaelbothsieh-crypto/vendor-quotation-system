...
## Task 3 Code Review 修復報告 2

### 1. 修復實作內容
* **PUT API 統編格式驗證**：在 `PUT /api/vendors/[id]` 端點中，加入與 `POST` 端點一致的台灣統一編號（taxId）格式驗證（若有填寫，必須為 8 位純數字），否則回傳 400 錯誤。
* **加強整合測試**：在 `scripts/test-vendors.ts` 整合測試腳本中加入 `4b. 測試更新廠商統編格式驗證` 測試案例，模擬以無效的統一編號對既有廠商進行更新，驗證是否正確回傳 400 錯誤。

### 2. 測試與驗證結果
* **整合測試驗證**：執行 `npx tsx scripts/test-vendors.ts`，成功通過所有 9 項整合測試，其中包括了新增的 `4b` 欄位格式錯誤攔截測試。
